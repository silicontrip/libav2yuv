...
-(int)decodeNextAudio
{
	...
	// there must be another way to initialise the audio decoder than this.
	if (sampleCounter == 0)
	{
	...
	do {
		...
		if ([self compareSamplesRange:endCounter] >=0) {
			if (([self compareSamplesRange:sampleCounter]<0) && ([self compareSamplesRange:endCounter] >0))
			{
				// send partial frame
