...
- (id)initWithNSUserDefaults:(NSUserDefaults *)args
{
	...
	//NSLog (@"libav2yuvArguments initWithArguments");
	extensions=[args boolForKey:@"extensions"];
	audio=[args boolForKey:@"audio"];
	convert=[args boolForKey:@"convert"];
	
	if ([args stringForKey:@"interlace"]!=nil)
	{
		NSString *intString = [args stringForKey:@"interlace"];
	...
	if ([args stringForKey:@"frameRate"]!=nil) {
		
	//	NSLog(@"frame rate string: %@", [args stringForKey:@"frameRate"]);
		
		if( Y4M_OK != y4m_parse_ratio(&frameRate, [[args stringForKey:@"frameRate"] UTF8String]) ) {
			return nil;
		}
		
		
	}
	format=[args stringForKey:@"format"];
	
	if ([args stringForKey:@"aspect"]!=nil) {
		NSString *aspectString = [args stringForKey:@"aspect"];
		if( Y4M_OK != y4m_parse_ratio(&aspect, [aspectString UTF8String]) ) {
			if([aspectString compare:@"PAL"] == 0) {
				y4m_parse_ratio(&aspect, [@"128:117" UTF8String]);
			} else if ([aspectString compare:@"PAL_WIDE"] == 0) {
	...
	if ([args stringForKey:@"chroma"]!=nil) {
		chroma = y4m_chroma_parse_keyword([[args stringForKey:@"chroma"] UTF8String]);
		if (chroma == Y4M_UNKNOWN) {
			[self chromaModes];
			return nil;
		}
	}
	outFile=[args stringForKey:@"out"];
	range=[args stringForKey:@"range"];
	
	stream=[args integerForKey:@"stream"];
